...
PROJECTS = [
    ...
    {"file": "세부4.xlsx", "target": "SUB", "id": "sub4", "prefix": "u4", "nav": "🔥 세부4과제",
     "title": "세부4과제 · 2026년(1차년도) 수행 업무",
     "sub": "수소–순산소 고효율 발전 시스템 원천기술 개발 — 신규(2026.07 착수) 1차년도 수행 업무·일정 · 4세부 연구개발계획서(2026.07~2029.06) 기반",
     "chips": ["🧩 <b>세부4</b> 과제 · 신규", "🤝 주관 <b>KIMM</b> · 공동 <b>KAIST·한울항공기계</b> · 해외 <b>DLR</b>", "🗓️ 1차년도 <b>2026.7~2027.6</b>"]},
]
...
SEED_MILE = {
    "chong": [
        {"name": "국제공동워크숍(KIMM 개최)", "date": "2026-08-05", "cat": "국제협력", "note": "DLR 참석"},
        {"name": "수요기업 사업화 설명회", "date": "2026-08-26", "cat": "성과관리", "note": "가스터빈 심포지움 2026 (8.26~28)"},
        {"name": "운영위원회", "date": "2026-09-15", "cat": "운영·관리", "note": "연 1회 이상"},
        {"name": "2차년도 연차실적보고서 제출", "date": "2027-02-15", "cat": "평가·보고", "note": "KETEP"},
    ],
    "sub1": [], "sub2": [], "sub3": [],
}
...
def read_mile(ws):
    out = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        name, date, cat, note = (list(row) + [None] * 4)[:4]
        if not _s(name):
            continue
        out.append({"name": _s(name), "date": fmt_date(date) if is_date(date) else "",
                    "cat": _s(cat), "note": _s(note)})
    return out
...
def read_budget(ws):
    out = []
    for row in ws.iter_rows(min_row=2, values_only=True):
        name, gov, inst, total, note = (list(row) + [None] * 5)[:5]
        if not _s(name):
            continue
        out.append({"name": _s(name), "gov": _num(gov), "inst": _num(inst), "total": _num(total)})
    return out
...
def emit_mile(miles):
    rows = ["    { name:%s, date:%s, cat:%s, note:%s }" % (js(m["name"]), js(m["date"]), js(m.get("cat", "")), js(m["note"]))
...
def emit_budget(b):
    rows = ["    { name:%s, gov:%d, inst:%d, total:%d }" % (js(x["name"]), x["gov"], x["inst"], x["total"]) for x in b]
    return "[\n" + ",\n".join(rows) + " ]" if rows else "[]"


def build_budget_js(b):
    return "const BUDGET = " + emit_budget(b) + ";"
...
def write_xlsx(path, cats, with_juk, perf, quant, miles):
    ...
    bd4 = _hdr_row(ws4, MILE_HEADERS, {3: "관련 업무 구분명(예: 국제협력) — 그 카테고리 바 위에 ★ 표시"})
    r = 2
    for m in miles:
        ...
        ws4.cell(r, 3, m.get("cat", ""))
        ws4.cell(r, 4, m.get("note", ""))
        for col in range(1, 5):
            cc = ws4.cell(r, col); cc.font = body; cc.border = bd4
            cc.alignment = wrap if col in (1, 4) else center
        r += 1
    for i, w in enumerate([34, 14, 14, 26], 1):
...
def do_sync():
    ...
    gp = os.path.join(DATA, "총괄.xlsx")
    budget = []
    if os.path.exists(gp):
        wbg = load_workbook(gp)
        if BUDGET_SHEET in wbg.sheetnames:
            budget = read_budget(wbg[BUDGET_SHEET])
    html = inject(html, "BUDGET", build_budget_js(budget))
    print(f"  연구비: 과제 {len(budget)}건")
